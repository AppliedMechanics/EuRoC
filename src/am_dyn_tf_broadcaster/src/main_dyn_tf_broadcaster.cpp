...
void update_tf(const ros::TimerEvent& event)
{
	...
	q_lwr.setRPY(0,0,configuration.q[0]);
	A_LWR_10.setRotation(q_lwr);
	// Joint 2
	A_LWR_21.setOrigin(tf::Vector3(0,0,0));
	q_lwr.setRPY(1.57079632679,0,configuration.q[1]);
	A_LWR_21.setRotation(q_lwr);
	// Joint 3
	A_LWR_32.setOrigin(tf::Vector3(0,0.2,0));
	q_lwr.setRPY(-1.57079632679,0,configuration.q[2]);
	A_LWR_32.setRotation(q_lwr);
	// Joint 4
	A_LWR_43.setOrigin(tf::Vector3(0,0,0.2));
	q_lwr.setRPY(-1.57079632679,0,configuration.q[3]);
	A_LWR_43.setRotation(q_lwr);
	// Joint 5
	A_LWR_54.setOrigin(tf::Vector3(0,-0.2,0));
	q_lwr.setRPY(1.57079632679,0,configuration.q[4]);
	A_LWR_54.setRotation(q_lwr);
	// Joint 6
	A_LWR_65.setOrigin(tf::Vector3(0,0,0.19));
	q_lwr.setRPY(1.570796326790,0,configuration.q[5]);
	A_LWR_65.setRotation(q_lwr);
	// Joint 7
	A_LWR_76.setOrigin(tf::Vector3(0,0,0.0));
	q_lwr.setRPY(-1.570796326790,0,configuration.q[6]);
	...
	//		if (ros::service::waitForService(forward_kin,ros::Duration(5.0)) && !call_active)
	//		{
	//			forward_kin_srv.request.configuration = configuration;
	//			if(!forward_kin_client.call(forward_kin_srv))
	//			{
	//				msg_error("errror in dk call");
	//				return;
	//			}
	...
	br.sendTransform(tf::StampedTransform(A_LWR_76, ros::Time::now(),lwr_6,LWR_TCP));
