...
template<typename PointT>
  ShapeGenerator<PointT>::ShapeGenerator() :
      noise_(0.0f), n_dist_(0.0f, 1.0f), n_rng_(rng_, n_dist_)
  {
  }
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateLine(const float & step_size, const Eigen::Vector3f & origin,
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateCircle(const float & step_size, const Eigen::Vector3f & origin,
                                              const Eigen::Vector3f & rotation_axis, const Eigen::Vector3f & radius,
                                              const float & angle)
  {
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateCirclePlane(const float & step_size, const Eigen::Vector3f & origin,
                                                   const Eigen::Vector3f & rotation_axis, const float & radius)
  {
    generateCirclePlane(step_size, origin, rotation_axis, getArbitraryPerpendicularNormalized(rotation_axis) * radius);
  }

template<typename PointT>
  void ShapeGenerator<PointT>::generateCirclePlane(const float & step_size, const Eigen::Vector3f & origin,
                                                   const Eigen::Vector3f & rotation_axis,
                                                   const Eigen::Vector3f & radius, const float & angle)
  {
    float radius_norm = radius.norm();
...
template<typename PointT>
  void ShapeGenerator<PointT>::generatePlane(const float & step_size, const Eigen::Vector3f & origin,
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateEdge(const float & step_size, const Eigen::Vector3f & origin,
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateEdge(const float & step_size, const Eigen::Vector3f & origin,
                                            const Eigen::Vector3f & direction_edge, const Eigen::Vector3f & direction_1,
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateCorner(const float & step_size, const Eigen::Vector3f & origin,
                                              const Eigen::Vector3f & direction_1, const Eigen::Vector3f & direction_2,
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateBox(const float & step_size, const Eigen::Vector3f & origin,
                                           const Eigen::Vector3f & direction_1, const Eigen::Vector3f & direction_2,
                                           const float & corner_size, const float & height)
  {
...
#endif
        generateEdge(step_size, origin + direction_2 + corner_direction_1, edge_direction_1, corner_direction_3,
                     -corner_direction_2);
        generateEdge(step_size, origin + direction_3 + corner_direction_1, edge_direction_1, -corner_direction_3,
                     corner_direction_2);
        generateEdge(step_size, origin + direction_2 + direction_3 + corner_direction_1, edge_direction_1,
                     -corner_direction_2, -corner_direction_3);
      }
      if (enable_edge_2)
      {
#ifndef REDUCE_BOX
...
#endif
        generateEdge(step_size, origin + direction_1 + corner_direction_2, edge_direction_2, -corner_direction_1,
                     corner_direction_3);
        generateEdge(step_size, origin + direction_3 + corner_direction_2, edge_direction_2, corner_direction_1,
                     -corner_direction_3);
        generateEdge(step_size, origin + direction_3 + direction_1 + corner_direction_2, edge_direction_2,
                     -corner_direction_3, -corner_direction_1);
      }
      if (enable_edge_3)
      {
#ifndef REDUCE_BOX
...
#endif
        generateEdge(step_size, origin + direction_1 + corner_direction_3, edge_direction_3, corner_direction_2,
                     -corner_direction_1);
        generateEdge(step_size, origin + direction_2 + corner_direction_3, edge_direction_3, -corner_direction_2,
                     corner_direction_1);
        generateEdge(step_size, origin + direction_1 + direction_2 + corner_direction_3, edge_direction_3,
                     -corner_direction_1, -corner_direction_2);
      }
...
#endif
      generateCorner(step_size, origin + direction_1, -corner_direction_1, corner_direction_2, corner_direction_3);
      generateCorner(step_size, origin + direction_2, corner_direction_1, -corner_direction_2, corner_direction_3);
      generateCorner(step_size, origin + direction_3, corner_direction_1, corner_direction_2, -corner_direction_3);

      generateCorner(step_size, origin + direction_1 + direction_2, -corner_direction_1, -corner_direction_2,
                     corner_direction_3);
      generateCorner(step_size, origin + direction_2 + direction_3, corner_direction_1, -corner_direction_2,
                     -corner_direction_3);
      generateCorner(step_size, origin + direction_1 + direction_3, -corner_direction_1, corner_direction_2,
                     -corner_direction_3);

      generateCorner(step_size, origin + direction_1 + direction_2 + direction_3, -corner_direction_1,
                     -corner_direction_2, -corner_direction_3);
...
#endif
        generatePlane(step_size, origin + direction_3 + corner_direction_1 + corner_direction_2, edge_direction_1,
                      edge_direction_2);
      }
...
#endif
        generatePlane(step_size, origin + direction_2 + corner_direction_3 + corner_direction_1, edge_direction_3,
                      edge_direction_1);
      }
    }

    else if (compound_shape == true)
    {
      Eigen::Vector3f edge_direction_1;
      Eigen::Vector3f edge_direction_2;
      Eigen::Vector3f edge_direction_3;
      ...
      if (enable_edge_1)
      {
        generateEdge(step_size, origin + corner_direction_1, edge_direction_1, corner_direction_2, corner_direction_3);
        generateEdge(step_size, origin + direction_2 + corner_direction_1, edge_direction_1, corner_direction_3,
                     -corner_direction_2);
        generateEdge(step_size, origin + direction_3 + corner_direction_1, edge_direction_1, -corner_direction_3,
                     corner_direction_2);
      ...
      if (enable_edge_2)
      {
        generateEdge(step_size, origin + corner_direction_2, edge_direction_2, corner_direction_3, corner_direction_1);
        generateEdge(step_size, origin + direction_1 + corner_direction_2, edge_direction_2, -corner_direction_1,
                     corner_direction_3);
        generateEdge(step_size, origin + direction_3 + corner_direction_2, edge_direction_2, corner_direction_1,
                     -corner_direction_3);
      ...
      if (enable_edge_3)
      {
        generateEdge(step_size, origin + corner_direction_3, edge_direction_3, corner_direction_1, corner_direction_2);
        generateEdge(step_size, origin + direction_1 + corner_direction_3, edge_direction_3, corner_direction_2,
                     -corner_direction_1);
        generateEdge(step_size, origin + direction_2 + corner_direction_3, edge_direction_3, -corner_direction_2,
                     corner_direction_1);
      ...
      // Generate corners:
      generateCorner(step_size, origin, corner_direction_1, corner_direction_2, corner_direction_3);
      generateCorner(step_size, origin + direction_1, -corner_direction_1, corner_direction_2, corner_direction_3);
      generateCorner(step_size, origin + direction_2, corner_direction_1, -corner_direction_2, corner_direction_3);
      generateCorner(step_size, origin + direction_3, corner_direction_1, corner_direction_2, -corner_direction_3);
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateCylinder(const float & step_size, const Eigen::Vector3f & origin,
                                                const Eigen::Vector3f & direction, const float & radius)
  {
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateCylinder(const float & step_size, const Eigen::Vector3f & origin,
                                                const Eigen::Vector3f & direction, const Eigen::Vector3f & radius,
                                                const float & angle)
  {
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateCone(const float & step_size, const Eigen::Vector3f & origin,
                                            const Eigen::Vector3f & direction, const float & radius_base,
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateSphere(const float & step_size, const Eigen::Vector3f & center,
                                              const Eigen::Vector3f & rotation_axis, const float & radius,
                                              const float & angle)
  {
    ...
    for (int step = 0; step <= radius_steps; ++step)
    {
      ...
      generateCircle(step_size, center + curr_point, rotation_axis_vector, curr_radius, 2.0f * (float)M_PI);
    }
  }
...
template<typename PointT>
  void ShapeGenerator<PointT>::generateHandle(const float & step_size, const Eigen::Vector3f & origin,
                                              const Eigen::Vector3f & rotation_axis,
                                              const Eigen::Vector3f & radius_curvature, const float & radius_handle,
                                              const float & angle)
  {
...
template<typename PointT>
  void ShapeGenerator<PointT>::addSinglePoint(const Eigen::Vector3f & point)
  {
    PointT p;
    p.x = point.x() + random();
    p.y = point.y() + random();
